    ...
    const $ = function (selector) {
        ...
        if (isString$1(selector)) {
            ...
            if (html[0] === '<' && html[html.length - 1] === '>') {
                ...
                each$1(tags, (childTag, parentTag) => {
                    if (html.indexOf(`<${childTag}`) === 0) {
                        toCreate = parentTag;
                        return false;
                    }
...
function getHandlers$1(element, type, func, selector) {
    ...
    return (handlers$1[getElementId$1(element)] || []).filter((handler) => handler &&
        (!event.type || handler.type === event.type) &&
...
function add$1(element, types, func, data, selector) {
    const elementId = getElementId$1(element);
...
function remove$1(element, types, func, selector) {
    const handlersInElement = handlers$1[getElementId$1(element)] || [];
    const removeEvent = (handler) => {
        delete handlersInElement[handler.id];
        element.removeEventListener(handler.type, handler.proxy, false);
    };
    if (!types) {
        ...
        types.split(' ').forEach((type) => {
            if (type) {
                getHandlers$1(element, type, func, selector).forEach((handler) => removeEvent(handler));
            }
        });
    }
}
...
function extend$1(target, object1, ...objectN) {
    objectN.unshift(object1);
    each$1(objectN, (_, object) => {
        each$1(object, (prop, value) => {
...
function mergeOptions$1(options) {
    ...
    // globalOptions 中的回调函数不合并
    each$1(globalOptions$1, (key, value) => {
        const callbacks = [
            'beforeSend',
            'success',
            'error',
...
function ajax$1(options) {
    ...
    const eventParams = {};
    // 参数合并
    const mergedOptions = mergeOptions$1(options);
    let url = mergedOptions.url || window.location.toString();
    const method = mergedOptions.method.toUpperCase();
    let data = mergedOptions.data;
    const processData = mergedOptions.processData;
    ...
    function trigger(event, params, callback, ...args) {
        ...
        if (callback) {
            ...
            if (callback in globalOptions$1) {
                // @ts-ignore
                result1 = globalOptions$1[callback](...args);
            }
            // 自定义回调
            if (mergedOptions[callback]) {
                // @ts-ignore
    ...
    function XHR() {
        ...
        return new Promise((resolve, reject) => {
            ...
            xhr.onload = function () {
                ...
                // statusCode
                each$1([globalOptions$1.statusCode, statusCode], (_, func) => {
                    if (func && func[xhr.status]) {
                        if (isHttpStatusSuccess) {
                            func[xhr.status](responseData, textStatus, xhr);
                        }
...
const dataNS$1 = '_mduiElementDataStorage';
...
function data$1(element, key, value) {
    ...
    // data(element, 'key', 'value')
    if (!isUndefined$1(value)) {
        setObjectToElement$1(element, { [key]: value });
        return value;
    }
    // 获取所有值
    // data(element)
...
$$1.extend = function (...objectN) {
    if (objectN.length === 1) {
        each$1(objectN[0], (prop, value) => {
            this[prop] = value;
        });
        return this;
    }
    return extend$1(objectN.shift(), objectN.shift(), ...objectN);
};

function map$1(elements, callback) {
    let value;
    const ret = [];
    each$1(elements, (i, element) => {
        value = callback.call(window, element, i);
        if (value != null) {
            ret.push(value);
        }
...
function removeData(element, name) {
    ...
    if (isUndefined$1(name)) {
        ...
        each$1(name, (_, nameItem) => remove(nameItem));
    }
}
...
each$1(['add', 'remove', 'toggle'], (_, name) => {
    $$1.fn[`${name}Class`] = function (className) {
        ...
        return this.each((i, element) => {
            ...
            const classes = (isFunction$1(className)
                ? className.call(element, i, element.getAttribute('class') || '')
                : className)
                .split(' ')
                .filter((name) => name);
            each$1(classes, (_, cls) => {
                element.classList[name](cls);
            });
        });
    };
...
each$1(['insertBefore', 'insertAfter'], (nameIndex, name) => {
...
each$1(['before', 'after'], (nameIndex, name) => {
    $$1.fn[name] = function (...args) {
        ...
        return this.each((index, element) => {
            const targets = isFunction$1(args[0])
                ? [args[0].call(element, index, element.innerHTML)]
                : args;
            each$1(targets, (_, target) => {
                let $target;
...
each$1(ajaxEvents$1, (name, eventName) => {
...
$$1.fn.map = function (callback) {
    return new JQ$1(map$1(this, (element, i) => callback.call(element, i, element)));
};
...
each$1(['prepend', 'append'], (nameIndex, name) => {
    $$1.fn[name] = function (...args) {
        return this.each((index, element) => {
            const childNodes = element.childNodes;
            const childLength = childNodes.length;
            ...
            let contents = isFunction$1(args[0])
                ? [args[0].call(element, index, element.innerHTML)]
                : args;
            // 如果不是字符串，则仅第一个元素使用原始元素，其他的都克隆自第一个元素
            if (index) {
...
each$1(['appendTo', 'prependTo'], (nameIndex, name) => {
    $$1.fn[name] = function (target) {
        const extraChilds = [];
        const $target = $$1(target).map((_, element) => {
            const childNodes = element.childNodes;
            const childLength = childNodes.length;
            if (childLength) {
                return childNodes[nameIndex ? 0 : childLength - 1];
...
each$1(['attr', 'prop', 'css'], (nameIndex, name) => {
    function set(element, key, value) {
        // 值为 undefined 时，不修改
        if (isUndefined$1(value)) {
            return;
...
$$1.fn.children = function (selector) {
    const children = [];
    this.each((_, element) => {
        each$1(element.childNodes, (__, childNode) => {
...
each$1(['', 's', 'sUntil'], (nameIndex, name) => {
...
function dataAttr$1(element, key, value) {
    if (isUndefined$1(value) && element.nodeType === 1) {
        const name = 'data-' + toKebabCase$1(key);
        value = element.getAttribute(name);
        if (isString$1(value)) {
            try {
...
$$1.fn.data = function (key, value) {
    ...
    if (isUndefined$1(key)) {
        ...
        const element = this[0];
        const resultData = data$1(element);
        // window, document 上不存在 `data-*` 属性
        if (element.nodeType !== 1) {
            return resultData;
        }
...
$$1.fn.filter = function (selector) {
    ...
    if (isString$1(selector)) {
        return this.map((_, element) => $$1(element).is(selector) ? element : undefined);
    }
...
$$1.fn.has = function (selector) {
    const $targets = isString$1(selector) ? this.find(selector) : $$1(selector);
...
function get$1(element, name, funcIndex, includeMargin) {
    ...
    const value = parseFloat(getComputedStyleValue$1(element, name.toLowerCase()) || '0');
...
function set$1(element, elementIndex, name, funcIndex, includeMargin, value) {
    let computedValue = isFunction$1(value)
...
each$1(['Width', 'Height'], (_, name) => {
    each$1([`inner${name}`, name.toLowerCase(), `outer${name}`], (funcIndex, funcName) => {
        $$1.fn[funcName] = function (margin, value) {
            // 是否是赋值操作
            const isSet = arguments.length && (funcIndex < 2 || !isBoolean$1(margin));
            const includeMargin = margin === true || value === true;
            // 获取第一个元素的值
            if (!isSet) {
                return this.length
...
each$1(['val', 'html', 'text'], (nameIndex, name) => {
    const props = {
        0: 'value',
        1: 'innerHTML',
        2: 'textContent',
    ...
    function get($elements) {
        ...
        if (nameIndex === 2) {
            // @ts-ignore
            return map$1($elements, (element) => toElement$1(element)[propName]).join('');
        }
        // 空集合时，val() 和 html() 返回 undefined
        if (!$elements.length) {
            return undefined;
        ...
        if (nameIndex === 0 && $$1(firstElement).is('select[multiple]')) {
            return map$1($$1(firstElement).find('option:checked'), (element) => element.value);
        }
        // @ts-ignore
        return firstElement[propName];
    }
    ...
    $$1.fn[name] = function (value) {
        ...
        // 设置值
        return this.each((i, element) => {
            const computedValue = isFunction$1(value)
            ...
            if (nameIndex === 0 && Array.isArray(computedValue)) {
                ...
                if ($$1(element).is('select[multiple]')) {
                    map$1($$1(element).find('option'), (option) => (option.selected =
                        computedValue.indexOf(option.value) >
                            -1));
                }
                // 其他 checkbox, radio 等元素
...
each$1(['', 'All', 'Until'], (nameIndex, name) => {
...
function set$1$1(element, value, index) {
    ...
    const currentOffset = get$1$1(element);
 ...
 * 标题
 */
class AlignCenter extends MenuAbstract {
    constructor() {
        super(...arguments);
        //static disable = ['bold', 'italic', 'image'];
        this.active = false;
 ...
 * 标题
 */
class AlignRight extends MenuAbstract {
    constructor() {
        super(...arguments);
        //static disable = ['bold', 'italic', 'image'];
        this.active = false;
...
class UploadFile extends MenuAbstract {
    constructor(editor, $button) {
        super(editor, $button);
        /**
         * <input type="file"/> 元素
    ...
    upload(file) {
        ...
        ajax({
            ...
            .then((response) => {
            //上传失败
            if (this.editor.options.fileUploadResponseTransform) {
                response = this.editor.options.fileUploadResponseTransform(response);
            }
            if (response.code) {
                mdui.alert(response.message);
                return;
            ...
            mdui.prompt('请输入文件名：', function (value) {
                ...
                console.log(value);
                //上传成功
                const $rootElem = that.selection.getRootElem();
                const rootHTML = $rootElem.html().toLowerCase().trim();
                const imgHTML = `<div onclick="window.open('${response.data.url}')" class="mdui-chip mdui_editor-chip"><span class="mdui-chip-title">${value}</span></>`;
                if ($rootElem[0].nodeName === 'P' &&
                    (rootHTML === '<br>' || rootHTML === '<br/>')) {
                    // 当前为空的 p 元素，替换该元素
